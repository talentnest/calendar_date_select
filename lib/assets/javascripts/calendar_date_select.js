...
CalendarDateSelect.prototype = {
  ...
  positionCalendarDiv: function() {
    var above = false, fixed = false;
    var c_dim = this.calendar_div.getDimensions(), c_height = c_dim.height, c_width = c_dim.width;

    var w_top = window.f_scrollTop(), w_height = window.f_height();

    var e = $(this.options.get("popup_by")),
        e_dim = e.cumulativeOffset(),
        e_top = e_dim.top,
        e_left = e_dim.left,
        e_height = $(this.options.get("popup_by")).getHeight(),
        e_bottom = e_top + e_height;

    if (this.options.get('offset'))
    {
      e_top += this.options.get('offset')[0];
      e_left += this.options.get('offset')[1];
    }
  ...
  initCalendarDiv: function() {
    if (this.options.get("embedded")) {
      ...
      var parent = $(this.options.get('container') || document.body);
